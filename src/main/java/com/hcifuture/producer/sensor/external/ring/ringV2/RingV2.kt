        ...
        connectJob = scope.launch {
            try {
                ...
                val serviceMap = mutableMapOf<UUID, List<UUID>>(
                    RingV2Spec.SERVICE_UUID to mutableListOf(
                        RingV2Spec.READ_CHARACTERISTIC_UUID,
                        RingV2Spec.WRITE_CHARACTERISTIC_UUID
                    )
                )
                connection = MyBleManager.connect(context, address, serviceMap) {
                    it.connectionObserver = this@RingV2
                    it.setMtu(247)
                }
                ...
                readJob = scope.launch {
                    connection?.getNotificationFlow(
                        RingV2Spec.SERVICE_UUID,
                        RingV2Spec.READ_CHARACTERISTIC_UUID
                    )?.collect {
                        val value = it.value ?: return@collect
                        val cmd = value[2]
                        val subCmd = value[3]
                        when {
                            cmd == 0x11.toByte() && subCmd == 0x0.toByte() -> {
                                ...
                                _statusFlow.emit(
                                    RingV2StatusData(
                                        ...
                                        softwareVersion = value.slice(4 until value.size)
                                            .toString(),
                                    )
                                )
                            }
                            ...
                            cmd == 0x40.toByte() && subCmd == 0x06.toByte() -> {
                                // imu
                                val data = value.slice(5 until value.size)
                                    .chunked(2)
                                    .map { (l, h) ->
                            ...
                            cmd == 0x61.toByte() && subCmd == 0x01.toByte() -> {
                                ...
                                _touchRawFlow.emit(
                                    RingV2TouchRawData(
                                        data = value.slice(5 until value.size),
                                        timestamp = System.currentTimeMillis(),
                                    )
                                )
                            }
                            ...
                            cmd == 0x61.toByte() && subCmd == 0x02.toByte() -> {
                                val event = if (value[4].toInt() == 0) {
                                    RingTouchEvent.TAP
                                } else if (value[4].toInt() == 1) {
                                    RingTouchEvent.HOLD
                                } else if (value[4].toInt() == 2) {
                                    RingTouchEvent.DOUBLE_TAP
                                } else if (value[4].toInt() == 3) {
                                    RingTouchEvent.DOWN
                                } else if (value[4].toInt() == 4) {
                                    RingTouchEvent.UP
                                } else {
                            ...
                            cmd == 0x71.toByte() && subCmd == 0x00.toByte() -> {
                                // microphone
                                val length = value[4].toInt().and(0xFF) or value[5].toInt().shl(8)
                                val sequenceId =
                                    value[6].toInt().and(0xFF) or value[7].toInt().and(0xFF)
                                        .shl(8) or
                                            value[8].toInt().and(0xFF).shl(16) or value[9].toInt()
                                        .shl(24)
                                _audioFlow.emit(
                                    RingV2AudioData(
                                        length = length,
                                        sequenceId = sequenceId,
                                        data = value.slice(10 until value.size),
                                    )
                                )
                            }
                            ...
                            cmd == 0x31.toByte() -> {
                                _ppgFlow.emit(
                                    RingV2PPGData(
                                        type = subCmd.toInt(),
                                        raw = value.slice(4 until value.size)
                                    )
                                )
                            }
...
//        commandJob?.cancel()
    ...
    suspend fun write(data: ByteArray) {
        try {
            connection?.writeData(RingV2Spec.SERVICE_UUID, RingV2Spec.WRITE_CHARACTERISTIC_UUID, Data(data), BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE)
        }
        catch (e: Exception) {
            Log.e("Nuix", "Error $e")
        }
